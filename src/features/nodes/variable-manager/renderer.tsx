import type { NodeProps } from "@xyflow/react";
import { Variable, Plus, Trash2, X } from "lucide-react";
import { useState } from "react";

import { useVariablesStore } from "@/features/variables/store";
import type { VariableManagerNodeData } from "./schema";
import { variableManagerNode } from "./index";
import { NodeFrame } from "@/features/nodes/presentation/components/node-frame";

export function VariableManagerNodeRenderer({ selected }: NodeProps & { data: VariableManagerNodeData }) {
    const { variables, addVariable, removeVariable } = useVariablesStore();
    const [newVarName, setNewVarName] = useState("");
    const [isAdding, setIsAdding] = useState(false);

    const handleAdd = () => {
        const trimmed = newVarName.trim();
        if (trimmed) {
            addVariable(trimmed);
            setNewVarName("");
            setIsAdding(false);
        }
    };

    return (
        <NodeFrame
            selected={selected}
            icon={<Variable size={16} />}
            title="Variable Manager"
            popoverTitle="Manage Variables"
            description={variableManagerNode.config.description}
            summary={`${variables.length} global variables`}
            showPopover={selected}
            showTargetHandle={false}
            showBottomHandle={false}
            compactClassName="w-[220px] min-h-[85px]"
            popoverClassName="w-[360px]"
            popoverBody={
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-tight">Global Variables</label>
                        <button
                            onClick={() => setIsAdding(true)}
                            className="flex items-center gap-1 text-[10px] font-bold text-[var(--ey-yellow)] hover:brightness-90 transition-colors uppercase"
                        >
                            <Plus size={12} />
                            Add Variable
                        </button>
                    </div>

                    {isAdding && (
                        <div className="flex items-center gap-2 bg-muted/20 p-2 rounded-lg border border-[var(--border-dim)]">
                            <input
                                type="text"
                                autoFocus
                                placeholder="new_variable_name"
                                className="flex-1 bg-transparent border-none text-xs focus:outline-none focus:ring-0 text-foreground"
                                value={newVarName}
                                onChange={(e) => setNewVarName(e.target.value)}
                                onKeyDown={(e) => e.key === "Enter" && handleAdd()}
                            />
                            <button
                                onClick={handleAdd}
                                className="bg-[var(--ey-yellow)]/20 text-foreground p-1.5 rounded-md hover:bg-[var(--ey-yellow)]/30 transition-colors"
                            >
                                <Plus size={14} />
                            </button>
                            <button
                                onClick={() => setIsAdding(false)}
                                className="text-muted-foreground p-1.5 hover:text-destructive transition-colors"
                            >
                                <X size={14} />
                            </button>
                        </div>
                    )}

                    <div className="flex flex-col gap-2 max-h-[320px] overflow-y-auto pr-1 select-none">
                        {variables.length === 0 && !isAdding ? (
                            <div className="text-center py-6 text-muted-foreground border border-dashed border-[var(--border-dim)] rounded-lg bg-muted/10">
                                <Variable className="mx-auto h-7 w-7 opacity-30 mb-2" />
                                <p className="text-xs">No variables defined</p>
                            </div>
                        ) : (
                            variables.map((variable) => (
                                <div
                                    key={variable.id}
                                    className="group/var flex items-center justify-between p-2.5 rounded-lg border border-[var(--border-dim)] bg-muted/20 hover:bg-muted/40 transition-colors"
                                >
                                    <div className="flex items-center gap-2 overflow-hidden">
                                        <span className="text-[10px] font-bold text-[var(--ey-yellow)] shrink-0">@</span>
                                        <span className="text-sm font-medium truncate">{variable.name}</span>
                                    </div>
                                    <button
                                        onClick={() => removeVariable(variable.name)}
                                        className="text-muted-foreground opacity-60 hover:opacity-100 hover:text-destructive transition-all"
                                    >
                                        <Trash2 size={14} />
                                    </button>
                                </div>
                            ))
                        )}
                    </div>
                </div>
            }
        />
    );
}
